...
const NavList = ({ children }: { children: React.ReactNode }) => {
  return <ul className='flex flex-col gap-9'>{children}</ul>
}
...
const NavGroup = ({
  ...
  return (
    <li className='flex list-none flex-col '>
      <h4 className='mb-1 pl-3 text-sm font-semibold'>{label}</h4>
  ...
  return (
    <li className='flex list-none items-center'>
      <Link
        href={href}
        className={cn(
          'h-full w-full rounded-full py-3 pl-3 text-[.9rem] hover:bg-cyan-100 dark:hover:bg-sky-700',
          href === pathname &&
